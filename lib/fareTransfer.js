...
class FareTransfer extends Contract {
    ...
    async InitLedger(ctx) {
        ...
        const fare= {
         ...
         await ctx.sub.putState(customers[0].LastTxnId, Buffer.from(stringify(sortKeysRecursive(fare))))
    }
    ...
    async GetCustomer(ctx, id) {
        ...
        if (!customerJSON || customerJSON.length === 0 || customerJSON.docType !== 'customer') {
    ...
    async ChargeFare(ctx, id, transitId) {
        ...
        let customer={}
        try{
            customer = await this.GetCustomer(ctx, id);
        }catch(err){
            return err;
        }
        ...
        if (lastTxnId && lastTxnId !== '') {
            try{
                lastFare = await this.GetCustomerLastFare(ctx, lastTxnId);
